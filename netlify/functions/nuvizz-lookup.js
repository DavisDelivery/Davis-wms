...
// Try these in order until auth succeeds
const URL_CANDIDATES = [
  'https://login.nuvizz.com/api-gateway/webservices/nudeliverit/v5',
  'https://login.nuvizz.com/api-gateway/webservices/nudeliverit/v7',
  'https://login.nuvizz.com/deliverit/openapi/v7',
  'https://login.nuvizz.com/deliverit/openapi/v5',
];
...
async function getToken() {
  if (cachedToken && Date.now() < tokenExpiry && workingBase) {
  ...
  for (const base of urls) {
    ...
    try {
      const res = await req(url, { headers: { 'Authorization': `Basic ${basic}` } });
      tried.push({ url, status: res.status, body: res.body.slice(0, 300) });
      console.log(`[AUTH] ${url} → ${res.status}`);
      ...
      if (res.status === 200) {
        ...
        if (data.authToken) {
          ...
          console.log(`[AUTH] SUCCESS → ${base}`);
          return { token: cachedToken, base: workingBase };
        }
        if (data.reasons) return { error: 'Rejected', reasons: data.reasons, tried };
      }
    } catch (e) {
      tried.push({ url, status: 0, error: e.message });
    }
  }
...
async function lookupStop(stopNbr) {
  ...
  const url = `${auth.base}/stop/info/${encodeURIComponent(stopNbr)}/${encodeURIComponent(COMPANY)}`;
  console.log(`[LOOKUP] ${url}`);
  const res = await req(url, { headers: { 'Authorization': `Bearer ${auth.token}` } });
  console.log(`[LOOKUP] ${res.status} ${res.body.slice(0, 500)}`);

  if (res.status === 404 || res.status === 409) {
    return { error: 'not_found', pro: stopNbr, message: `Stop not found (HTTP ${res.status})`, apiUrl: url, detail: res.body.slice(0, 300), source: 'nuvizz_live' };
  }
  if (res.status === 401) {
    ...
    const res2 = await req(`${auth2.base}/stop/info/${encodeURIComponent(stopNbr)}/${encodeURIComponent(COMPANY)}`, { headers: { 'Authorization': `Bearer ${auth2.token}` } });
    if (res2.status !== 200) return { error: 'retry_failed', httpStatus: res2.status, detail: res2.body.slice(0, 300), source: 'nuvizz_live' };
    return parse(JSON.parse(res2.body), stopNbr);
  }
  if (res.status !== 200) return { error: 'api_error', pro: stopNbr, httpStatus: res.status, detail: res.body.slice(0, 500), apiUrl: url, source: 'nuvizz_live' };

  let data; try { data = JSON.parse(res.body); } catch (e) { return { error: 'parse_error', detail: res.body.slice(0, 500), source: 'nuvizz_live' }; }
  return parse(data, stopNbr);
}
...
function parse(data, pro) {
  ...
  const s = view.stop || {};
  const l = view.load || {};
  const x = view.stopExecutionInfo || {};
...
function ft(d) {
  if (!d) return ''; try { const [,t]=d.split('T'); const [h,m]=t.split(':'); const hr=parseInt(h,10); return `${hr%12||12}:${m} ${hr>=12?'PM':'AM'}`; } catch(e){return d;}
}
...
exports.handler = async (event) => {
  ...
  if (!USERNAME || !PASSWORD) {
    const r = MOCK[pro] || { error:'not_found', pro, message:'Not in mock data (no NuVizz creds)', source:'mock' };
    return { statusCode: 200, headers: H, body: JSON.stringify(r) };
  }

  try {
    return { statusCode: 200, headers: H, body: JSON.stringify(await lookupStop(pro)) };
  } catch(e) {
    console.error('[FATAL]', e);
    return { statusCode: 200, headers: H, body: JSON.stringify({ error:'server_error', message:e.message, pro, source:'nuvizz_live' }) };
  }
};
